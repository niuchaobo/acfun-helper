 ...
 * 生放送优化
 */
class LivePageButfy extends AcFunHelperFgFrame {
    constructor() {
        super();
    ...
    widePlayerEvent() {
        $('div.box-right').on('click', '#toggleWide', (e) => {
            ...
            if (this.isWidePlayer) {
                ...
                document.querySelector("div.live-feed").classList.contains("dark-style") && this.darkMode(false)
            } else {
                this.darkMode(true)
    ...
    enterWidePlayerModel() {
        ...
        $("#wide-player-right").on('click', (e) => {
            if ($(".live-feed").css("display") === "none") {
                document.querySelector("div#toggleWide").click();
    ...
    darkMode(mode) {
        if (mode) {
            document.querySelector("div.gift-bar").classList.add("dark-style");
            document.querySelector("div.live-feed").classList.add("dark-style");
        } else {
            document.querySelector("div.gift-bar").classList.remove("dark-style");
            document.querySelector("div.live-feed").classList.remove("dark-style");
        }
    }

    wideDark() {
        $('.live-feed .face-text').append(DOMPurify.sanitize(`<i class="notice_icon" id="darkMsg" title="暗色模式" style="position:absolute; width:20px; left:100px; top:0px; cursor:pointer;"><svg t="1650104123251" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4253" width="20" height="20"><path d="M888.159701 678.959541c-127.792763 31.948191-261.975164-12.779276-357.819736-102.234211-127.792763-127.792763-153.351315-325.871545-57.506743-479.22286C504.781413 52.775003 460.053946-11.121378 402.547203 1.657898c-95.844572 19.168914-185.299506 70.28602-255.585526 140.572039C51.117105 238.074509 0 372.25691 0 506.439311 0 640.621712 51.117105 774.804113 153.351315 870.648685 249.195887 972.882895 376.98865 1024 517.560689 1024c140.572039 0 268.364802-51.117105 364.209374-153.351315 25.558553-25.558553 51.117105-57.506743 76.675658-95.844572 25.558553-44.727467-12.779276-108.623848-70.28602-95.844572z" p-id="4254" fill="#8a8a8a"></path><path d="M1003.173187 238.074509c25.558553-25.558553 25.558553-63.896381 0-89.454934-25.558553-25.558553-63.896381-25.558553-89.454933 0-19.168914 19.168914-83.065296 19.168914-102.234211 0-25.558553-25.558553-63.896381-25.558553-89.454934 0-25.558553 25.558553-25.558553 63.896381 0 89.454934 19.168914 19.168914 19.168914 83.065296 0 102.23421-25.558553 25.558553-25.558553 63.896381 0 89.454934 25.558553 25.558553 63.896381 25.558553 89.454934 0 19.168914-19.168914 83.065296-19.168914 102.234211 0 25.558553 25.558553 63.896381 25.558553 89.454933 0 25.558553-25.558553 25.558553-63.896381 0-89.454934-19.168914-19.168914-19.168914-83.065296 0-102.23421z" p-id="4255" fill="#8a8a8a"></path></svg></i>`));
        document.querySelector("#darkMsg").addEventListener("click", () => {
            if (document.querySelector("div.live-feed").classList.contains("dark-style")) {
                this.darkMode(false);
            } else {
                this.darkMode(true);
            }
    ...
    simplifyDanmu() {
        ...
        <li style="height: 35px;display: flex; align-items: center;"><input type="checkbox" data-type="emojiCommand">屏蔽表情包指令</input></li>
        <li style="height: 35px;display: flex; align-items: center;"><input type="checkbox" data-type="comment">屏蔽评论</input></li>
        </ul></div>`));
        ...
        $('#noticeBtn').on("click", (e) => {
            ...
            $('.hide_popup').css('display') === "none" ?
                $('.hide_popup').css({ left: e.pageX - 50 + 'px', top: e.pageY - 390 + 'px' }).show() :
                $('.hide_popup').hide()
        });
        document.querySelector(".hide_popup").addEventListener("dblclick", (e) => {
            if (e.target.nodeName === "INPUT") {
                let listElem = document.querySelector(".hide_popup").children[0].children;
                if (e.target.checked == false) {
                    const eventSource = e.target.dataset.type;
                    for (let i = 0; i < listElem.length; i++) {
                        if (listElem[i].children[0].dataset.type != eventSource) {
