...
const app = {
    template: (await importVue("pageHandler/SettingPanel/Article/BanUserInPart.vue")).template,
    data() {
        return {
            raw: {},
            memberNum: 0,
    ...
    methods: {
        ...
        removeUser: function (e) {
            ...
            mdui.snackbar({
                ...
                onClose: () => {
                    ExtOptions.setValue("UserFilter", this.raw);
                    this.memberNum--;
                },
            });
        },
        addUser: function () {
            mdui.prompt('请输入你需要关注的用户UID', '添加关注', async (e) => {
                userAddWrap(this.raw, e, (userInfo, uid) => {
                    ...
                    this.raw[uid] = name;
                    ExtOptions.setValue("UserFilter", this.raw).then(() => {
                        mdui.snackbar({
    ...
    mounted: async function () {
        batchOprtSwitches([{ key: "filter" }]);
        this.raw = await ExtOptions.getValue("UserFilter");
        if (!!this.raw) {
            await ExtOptions.setValue("UserFilter", {});
        }
...
export const banuserInpart = app